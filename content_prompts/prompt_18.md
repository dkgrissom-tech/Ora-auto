# Prompt 18 — Ora manual X waitlist draft

**Brand:** Ora  
**Platform(s):** X (manual only)  
**Pillar:** Manual social outreach  
**Cadence:** One reusable post; vary the hook and asset for each run without repeating a recent post.

## What to generate

Write a brief X draft that announces an upcoming Zara faceless demo and invites people to the Ora waitlist. The result still uses intake frontmatter so it can be stored, but it must be explicitly marked for manual posting.
...
- X is manual-only by policy: state in the body's final line `Manual posting note: post this on X by hand.`
- Use `platforms: [x]` in the intake draft. Do not imply the scheduler will publish it. Keep the actual post copy under 260 characters before the manual note.
...
brand: ora
platforms: [x]
scheduled: 2026-09-04T09:00:00-05:00
filename: ora-zara-demo-manual-x-2026-09-04.md
---

A clearer product demo does not need a talking head.

Zara, Ora's AI persona, is sharing an early faceless look at a clearer post-meeting workflow. Ora is still in waitlist mode—follow @meetora for the next demo.

Manual posting note: post this on X by hand.
```
